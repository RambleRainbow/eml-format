...
var emlformat = {
  ...
  getEmailAddress: function(raw) {
    ...
    if (match) {
      var name = emlformat.unquoteEncoding(match[1]).replace(/"/g, "").trim();
      if (name && name.length) {
        result.name = name;
      }
  ...
  unquoteEncoding: function(s) {
    var regex = /=\?(.+?)\?(B|Q)\?(.+?)(\?=)/gi;
    var match = regex.exec(s);
    if (match) {
      var codePage = match[1];
      var type = match[2].toUpperCase();
      var value = match[3];
      if (type == "B") { //Base64
        let buf = Buffer.from(value.replace(/\r?\n/g, ""), "base64");
        let txt =iconv.decode(buf, codePage);
        return txt; 
      }
  ...
  function _read(data) {
    try {
      ...
      if (data.headers["Subject"]) {
        result.subject = emlformat.unquoteEncoding(data.headers["Subject"]);
      }
      ...
      function _append(headers, content) {
        ...
        if (!result.html && contentType && contentType.indexOf("text/html") >= 0) {
          ...
          if (name) {
            attachment.name = emlformat.unquoteEncoding(name);
          }
